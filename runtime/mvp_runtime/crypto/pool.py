...
def assert_pool_within_size_cap(entries: Sequence[Mapping[str, Any]]) -> None:
    ...
    if len(occupying) > MAX_ROUTABLE_STRATEGIES:
        raise ToolError(
            ...
            "lineages than any of them can fill a 20-trade lifecycle window with, so nothing "
            "in it can be auto-demoted. Retire first (scripts/retire_strategies.py), or pass "
