...
`replaced_entries` stand on, hashes a spec the same way. A patch on `pool` does not reach the functions
here, which read this module's names: a test that means to change what they see patches `live_tier`
too, as the readiness test does for :func:`live_arm_unsound`.
"""
