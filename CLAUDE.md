...
- Next: all roadmap phases (R0.5–R8) plus R9 are done. Candidates: the deferred R4 `audit`/`recovery` console verbs; extracting the now-fourfold `select_gated(...)` chokepoint pattern (provider/search/operator/writer). **Approval consumption is NOT a normal increment** — it needs a governance change (two policy flags), an active-gate edit, and v0.2 bumps of `approval`/`permission_decision`; treat it as a separate, explicit Thomas decision.
