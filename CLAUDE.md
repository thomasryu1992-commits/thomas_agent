...
- Next: all roadmap phases (R0.5–R8) are done. Candidates: the deferred R4 items (control-channel approval handling — now the natural unblocker for APPROVAL_REQUIRED actions; `audit`/`recovery` console verbs), and extracting the now-fourfold `select_gated(...)` chokepoint pattern (provider/search/operator/writer).
