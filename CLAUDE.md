...
- Next: every roadmap phase (R0.5–R9) and both deferred R4 items are done. **Approval consumption is NOT a normal increment** — it needs a governance change (two policy flags), an active-gate edit, and v0.2 bumps of `approval`/`permission_decision`; treat it as a separate, explicit Thomas decision.
