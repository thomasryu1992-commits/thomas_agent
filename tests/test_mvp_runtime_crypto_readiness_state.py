...
def _status(*, opted=False, **overrides):
    ...
    inputs = {
        ...
        "tradable_armed": 1,
        "recorded_fire": {
            "known": True, "created_at": FIVE_MINUTES_AGO, "age_seconds": 300.0, "stale": False,
            "contexts": 13, "synthetic": 0, "degraded": 0, "optional_data": 0, "incident": [],
            "gate": {"confirmation_present": True, "manual_kill_switch": False}, "error": None,
        },
        "account": ({"source": "this_process", "configured": True, "readable": True, "error": None}
                    ...
                    {"source": "recorded", "configured": False, "recorded": True,
                     ...
                     "realized_net": 0.0, "daily_loss_breached": False}),
...
def test_the_components_are_the_ones_the_directive_names_and_the_runtime_control():
...
_FLIPS = {
    ...
    "snapshots": (dict(checks={"pre_order_snapshots": False}), "risk_ready", False,
                  "PRE_ORDER_SNAPSHOTS_UNREADABLE"),
...
def test_a_minority_of_degraded_contexts_is_not_a_market_data_failure():
    """The rule `pool_cycle_is_stalled` uses: half or more. One flaky feed of thirteen does not stop
    the other twelve contexts entering."""
    possible, state = _state(_status(inputs={"recorded_fire": {"degraded": 6}}))
    assert possible is True and state["components"]["market_data_ready"]["ok"] is True
...
def test_no_cycle_on_record_is_unknown_never_off():
    ...
    assert possible is None and state["blocking"] == []
    assert state["unknown"] == ["live_gate_open:NO_RECORD", "market_data_ready:NO_RECORD",
                                "reconciliation_ready:NO_RECORD"]
...
def test_an_unreadable_ledger_is_unknown_and_says_so():
    ...
    assert state["unknown"] == ["live_gate_open:RECORD_UNREADABLE", "market_data_ready:RECORD_UNREADABLE",
                                "reconciliation_ready:RECORD_UNREADABLE"]


def test_a_stale_record_is_no_recent_cycle_and_the_gate_is_unknown_not_off():
    """Entries happen only inside a cycle, so a trading process that has not recorded one for two
    hours opens nothing — False, as the four-fact answer said. But the gate itself is unknown: an old
    record is not evidence about now, and the env rows of a process that cannot see the env are not
    evidence either (FC-10)."""
    possible, state = _state(_status(recorded_gate={"stale": True, "age_seconds": 3 * 3600.0},
                                     inputs={"recorded_fire": {"stale": True, "age_seconds": 3 * 3600.0}}))
    assert possible is False
    assert state["components"]["live_gate_open"] == {"ok": None, "reason": "RECORD_STALE", "source": "recorded"}
    assert state["blocking"] == ["market_data_ready:NO_RECENT_CYCLE", "reconciliation_ready:NO_RECENT_CYCLE"]
...
def test_the_model_never_raises_and_never_says_true_on_an_empty_status():
    possible, state = _state({})
...
def test_the_data_line_fits_the_assistant_door_on_a_heavy_board():
    ...
    (`thomas_door_client.DATA_MAX_CHARS`); keys sort, so a cut loses `recorded_gate`, `readiness` and
    `venue_contract` first. Every component refusing, with a contract naming several symbols and
    failed checks, must still fit with room to spare."""
    status = _status(
        ...
        venue_contract={"error": None, "usable": False, "refusal": "ENTRY_CONTRACT_NOT_PASS",
                        "failed_checks": ["position_mode", "multi_assets_mode", "leverage_bracket",
                                          "symbol_filters", "order_types"],
        ...
        inputs={"runtime_control": {"mode": KILLED, "fail_closed": True},
                "recorded_fire": {"incident": ["BTCUSDT__1h", "ETHUSDT__4h"], "degraded": 13, "synthetic": 13,
    ...
    line = json.dumps(live_readiness.readiness_data(status), ensure_ascii=False,
                      separators=(",", ":"), sort_keys=True)
    assert len(line) < 3400, len(line)


# === the model, over the real board ===========================================================

def _write_fire(root, *, created_at, statuses=("HELD", "HELD"), gate=True, degraded=(), incident=(),
                synthetic=(), switches=None, optional=None):
    """One fire's cycle records, as the trading process writes them: one per routed context, all
    stamped with the fire's instant, each carrying the gate switches its live leg read. ``incident``
    names the contexts that halted; ``switches`` overrides one context's switches; ``optional`` maps a
    context to how its optional data failed (PR2d-2): ``stale``, ``missing`` or a degrade ``code``."""
...
def _ready_console_machine(root, monkeypatch):
    ...
    armed and active, a live-autonomous stage, a usable contract over the budget, a fresh fire and
    a fresh account snapshot."""
    ...
    path.write_text(json.dumps({"active_strategies": [
        {"strategy_id": "S1", "candidate_id": "c_S1", "status": "PAPER_ACTIVE", "strategy_spec": spec,
         pool_store.LIVE_TIER_FIELD: pool_store.LIVE_TIER_LIVE},
    ]}), encoding="utf-8")
    # The arm's approval pairing has its own tests (PR3a); here the arm is one the gate accepts.
    monkeypatch.setattr(pool_store, "live_arm_unsound", lambda entry: None)
...
def _write_snapshot(root, *, as_of, net):
    path = account_store.snapshot_path(root)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps({
        "record_type": account_store.RECORD_TYPE, "as_of": as_of, "written_at": as_of,
        "configured": True, "asset": "USDT", "wallet_balance": 500.0, "open_position_count": 0,
        "realized_windows": {"today": {"net": net}, "1d": {"net": net}},
    }), encoding="utf-8")
...
def test_a_stale_record_on_a_console_is_not_live_trading_off(tmp_path, clean_env, monkeypatch):
    ...
    assert data["live_entry_possible"] is False
    assert data["readiness"]["blocking"] == ["market_data_ready:NO_RECENT_CYCLE",
