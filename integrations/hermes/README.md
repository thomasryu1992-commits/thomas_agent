...
| `config/skills/thomas-ops/` | the `thomas-ops` skill (`SKILL.md` 1.4.1) and its references | Hermes skill |
...
| `config/cron-jobs.template.json` | the three cron jobs' configuration fields (schedule, prompt, model, toolsets); run state omitted; delivery target a placeholder | template |
