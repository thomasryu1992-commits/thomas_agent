...
> 이고 **강제된 시장 중립이 아니다.**
>
> **그 차이가 왜 승인 사안인지가 이제 판단이 아니라 산수로 확인됐다.** 값싼 형태 — cap 한도를
> 0까지 조여 북이 아예 기울지 못하게 하는 "거절로 강제하는 중립" — 은 일방향이라 새 게이트가
> 필요 없다. 그런데 **작동하지 않는다.** 정도의 문제가 아니라 구조의 문제다: **어떤 쌍이든 첫
> 다리 자체가 불균형**이므로, 한도 0에서는 **빈 북이 모든 엔트리를 거절**하고 북은 영원히
> 비어 있다. 짝을 이룬 북으로 가는 모든 경로가 짝이 없는 상태를 통과한다.
> (측정: 1,200봉 20컨텍스트에서 한도 4·2는 59개, 1은 51개, **0은 0개** / 거절 334건.)
>
> 즉 거절은 불균형의 **크기**를 묶을 수 있을 뿐 **짝을 만들 수는 없다.** 중립은 두 다리를
> **동시에** 여는 것으로만 도달 가능하고, 그건 런타임이 어떤 전략도 제안하지 않은 거래를
> 저작하는 일이다 — 심볼·방향·크기·스톱·타깃·보유기간·청산조건을 전부 게이트가 지어내야 한다.
> 그게 새 권위이고, 그래서 답이 "한도를 낮추면 된다"가 될 수 없다. `paper.py`의
> `test_enforced_neutrality_is_not_reachable_by_declining`이 이 산수를 고정한다.
>
