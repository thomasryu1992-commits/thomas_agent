...
**상태:** DRAFT 2026-08-12 — PR-2(판정 활성화) 결정 대기. PR-1(기록 전용)은 구현됐고, 결정 근거인 §4 측정 결과는
아직 기록되지 않았다. 날짜는 추정이다(FACTORY_ABLATION 결정 이후 작성).
**원래 상태(작성 당시):** DRAFT — PR-1(기록 전용)은 구현됨; **PR-2(판정 활성화)는 아래 §4의 측정 결과를 근거로
