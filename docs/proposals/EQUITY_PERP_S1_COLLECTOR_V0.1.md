...
**상태:** IMPLEMENTED 2026-08-04 — 구현·운영 중. `crypto/market_data.py`의 `hyperliquid` 수집기가
2026-08-04부터 매시간 `candle_archive`로 돈다(`docs/REMAINING_WORK.md` §H, H0에 첫날 결함과 수정).
날짜는 가동일이다.
**원래 상태(작성 당시, 2026-09-25에 위로 정정):** DRAFT — 설계 선행(구현 아님).
...
1h 데이터는 자체 적재 없이는 **영원히 도달 불가**다. 4h(5000봉 = 833일)와 1d는 여유가 있다.

**그리고 15m에서는 이미 일어났다 — 이것이 상한이 실재한다는 증거이자, 손실이 가정이 아니라는 증거다.**
`xyz:SP500`은 2026-03-18 상장이므로 15m 봉이 약 13,248개 있어야 한다. 실측 요청 결과:

| interval | 반환된 봉 | 커버 구간 | 존재하는 히스토리 |
|---|---|---|---|
| 1h | 3,304 | 2026-03-18 → 08-03 (**138일 전체**) | 상한 아래, 전량 반환 |
| 15m | **5,007** | 2026-06-12 → 08-03 (**52일뿐**) | **86일치가 이미 도달 불가** |

즉 5000은 "베뉴가 가진 전부"가 아니라 **실재하는 천장**이고, `SP500`의 2026-06-12 이전 15m 데이터는
**이미 API에서 사라졌다.** 1h는 약 70일 뒤 같은 일을 겪는다. 위험한 것은 1h 이하 전부이며,
그중 15m은 이미 손실 구간에 들어가 있다.
