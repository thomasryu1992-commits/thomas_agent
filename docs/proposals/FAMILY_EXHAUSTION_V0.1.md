...
소진 절(30일 CONFIRMED·마지막 CONFIRMED). 1h 5심볼 민팅은 이미 돌고 있었다(§6 정정, 2026-09-26).
...
**정정 (2026-09-26, 운영 `schedules.jsonl`·`strategy_candidates.jsonl` 대조):** "ETH·SOL·BNB·DOGE 1h에는
공급이 없는 셈"은 틀렸다. 이 절은 5심볼 코호트 스케줄 하나만 봤다. 단일 심볼 1h 스케줄
(`ETHUSDT 1h`·`BNBUSDT 1h`·`SOLUSDT 1h`·`DOGEUSDT 1h`, 2026-07-23 생성)이 켜진 채 매일 민팅하고 있다
(2026-09-26 GEN-958~961이 각각 그 심볼의 1h). 꺼진 것은 `BTCUSDT 1h` 단일 스케줄뿐이고, BTC 1h는
코호트 스케줄이 채운다. 위의 "248행이 전부 BTCUSDT"는 코호트 스케줄의 행에 대해서만 맞다.
코호트 발화는 5심볼 범위의 1h 트라이얼도 민팅한다(GEN-964) — 코호트를 끄면 그것도 멈춘다.
...
- **§6 / Q3의 확인 대기:** 08-24 "1h 라우팅 슬롯 9개 공급"의 의도는 5심볼 전부였다. 1h는 5심볼 모두에서 민팅하되
  풀링하지 않는다(시스템 점검 D4). 코호트 스케줄을 단일 심볼 스케줄 다섯 개로 나누자는 처방은 §6 정정대로
  이미 충족돼 있어 실행하지 않았다(Thomas 2026-09-26: 현재 상태 유지). 1h를 풀링하지 않는 이유는
