...
**권고.** forward 구간을 **해당 타임프레임의 홀드아웃 슬라이스 폭**(tail/10 유도식 그대로:
4h = 21일, 1d = 42일)의 달력 슬라이스로 나누고, `_periods_confirm`과 **동일한 검정**을
적용한다: 무거래 슬라이스는 제외(0 계상 금지), 활동 슬라이스 ≥ `MIN_HOLDOUT_PERIODS`(8),
슬라이스 평균이 자기 분산의 Student-t 95% 장벽(`t_critical_95`)을 클리어. 별도의 "최소
forward 일수" 상수는 만들지 않는다 — 8개 활동 슬라이스 요건이 곧 시계다(4h ≈ 최소 24주,
1d ≈ 최소 48주).
