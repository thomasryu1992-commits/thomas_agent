...
**상태:** IMPLEMENTED 2026-08-10 — D1·D2 채택, D3 방향 승인. PR-A(pipeline-worker, 문 전환)와 PR-B(스케줄러 Naver env
제거) 구현.

결정: **채택** (Thomas, 2026-08-10 — D1·D2 채택, D3 방향 승인, "응 그렇게 구현해줘").
작성 2026-08-10, 기준 `main` = `7561e1e`. 구현은 §6의 PR-A(pipeline-worker + 문 전환)와
PR-B(스케줄러 Naver env 제거)로 진행한다.
...
2. **`naver_research.py`를 호출하는 코드가 아직 없다.** Phase 1은 자기완결 모듈이고,
   ...
   **돌아가는 것을 하나도 끊지 않는다** — 순수한 토폴로지 + 테스트 이동이다.
   ...
   > **2026-08-10 갱신 (#666, 이 문서 작성 수 시간 뒤):** 전제 2는 좁아졌다. #666이
   > `run_task(keyword_seeds=...)` opt-in과 CLI `--naver-keywords`로 어댑터를 배선했다.
   > dispatch 경로는 seeds를 넘기지 않으므로 PR-A는 영향이 없지만, CLI는 관례상
   > scheduler 컨테이너에서 실행되므로 **PR-B는 CLI 키워드 실행 안내를
   > `thomas-pipeline-worker` 컨테이너로 옮기는 변경을 함께 싣는다** — 아니면 env가
   > 빠진 스케줄러 안의 CLI 실행이 조용히 mock으로 떨어진다(이 저장소가 #650에서 막은
   > 바로 그 실패 모양). 전제 1·3(라이선스 미취득, 게이트 unset → 전부 mock)은 유효하고,
   > 타이밍 논거도 그대로다.
