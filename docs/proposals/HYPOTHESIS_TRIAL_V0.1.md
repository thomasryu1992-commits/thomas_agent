...
않은 채 14일째 멈춰 있고**, 채택 근거는 **전부 FRAGILE·33개가 거래 0건·19개는 자기 타임프레임이
아닌 1h 봉으로 채점된 백테스트**다. 트라이얼 층을 짓기 전에 그 입력부터 고쳐야 하며(§5, 결정
...
- **채점 스냅샷은 항상 BTCUSDT 1h 하나**다(`schedule.request` 기본값, `collect_market_data`의 기본
  ...
  타임프레임만 읽고 둘을 비교하는 곳이 없다. 그래서 4h 제안의 `max_holding_bars=21`은 1h 봉
  21개(21시간)로 재생되고, 그 결과가 "채택"으로 기록된다. **이것은 결함이다**(§5 D-0).
- 거래 0건의 원인 중 하나(런타임이 열을 공급하지 않던 것)는 이미 고쳐졌다(`scheduler.py`
  proposer 분기의 `attach_mining_legs` 주석: 당시 22개 중 18개). 남은 원인은 창 깊이와 타임프레임
  불일치다.
...
- **D-0. proposer 채점 결함(별도 PR):** spec 타임프레임이 스냅샷과 다르면 채택하지 않는다
  ...
  자기 타임프레임으로 다시 수집해 채점하는 것은 발화당 요청 수를 늘리므로 C안의 일부로 둔다.
...
- **Q3. 트라이얼 forward:** (a) 별도 트랙(권고: 동결된 cohort 115명과 그 null 대조가 바뀌지 않음)
  (b) cohort 시더의 필터를 넓힘(Option A라 cohort 증거는 LIVE 문에 닿지 않지만, cohort와 null
  대조의 모집단이 섞인다).
