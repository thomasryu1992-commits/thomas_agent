# 제안: Hermes 스위치 문 — S1·S3 채택 후 출시, S2 거부, S4는 이후 출시

**상태:** IMPLEMENTED 2026-07-31 — S1·S3 채택·출시(#387), S2 거부. S4(자금 스냅샷)는 아래 표가 적은 뒤에 출시됐다
(#755: 스케줄러가 쓰고 read 문의 `crypto_funds`가 보여 준다). 레인은 policy 1.4.0에 조문화됐다.
...
> | **S4** 자금 스냅샷을 별건으로 | **미착수** — `read_bridge`의 명령 집합에 해당 verb 없음 (2026-08-03 확인). **이후 출시** — #755, `crypto_funds` (2026-09-26 확인) |
>
