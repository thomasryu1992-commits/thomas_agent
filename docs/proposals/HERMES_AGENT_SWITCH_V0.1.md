# 제안: Hermes 스위치 문 — 비서가 자동매매를 켜고 끌 수 있는가 (DRAFT v0.1)

> **상태: DRAFT — 결정을 위한 문서. 어떤 것도 활성화하지 않으며 구현 착수도 아니다.**
> 요구되는 Thomas 결정은 §8에 있다. §9의 정책 diff는 **초안이며
> `governance/GOVERNANCE_POLICY.yaml`에 적용되지 않았다.**
>
> 사실 확인 기준: `main` = `396c7ce` (2026-07-31). 정책 인용은 전부
> `governance/GOVERNANCE_POLICY.yaml`(`policy_version: 1.2.0`)에서 직접 읽었고, 런타임 상태는
> 이 호스트의 실제 컨테이너에서 조회했다. 이 문서는 정책의 authority가 아니다 — 정책을
> **바꾸자는 제안**이며, 바꿀지는 Thomas가 정한다.
...
| # | 결정 | 성격 |
|---|---|---|
| **S1** | 스위치 문을 만든다 (§6) — 요청은 Hermes, 승인은 인증 채널의 `/approve <id>` | 정책 변경 **없음** — 구현 승인 |
| **S2** | `enable`의 라이브 게이트 조건 분기(§6.2)를 채택한다 | 정책 조문 (§9) |
| **S3** | halt 문을 스위치 문의 `disable`로 흡수하고 문을 3개로 유지한다 | 구조 결정 |
| **S4** | 자금 스냅샷(§7)을 별도 PR로 진행한다 | 별건 승인 |

**권장: S1·S3는 지금, S2는 §9 조문과 함께, S4는 별건.**
