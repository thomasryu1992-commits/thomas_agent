...
**Status:** IMPLEMENTED — 제안 1~5 전부 landed: 제안 1 typed/versioned frame(PR7 #833), 제안 2 registry origin `AGENT`(PR8 #834), 제안 3 멱등 replay `{task_id, status, result}`(PR9 #835), 제안 4 read 문 조회 verb 4종 + `READ_VERB_AUTHORITY`(PR10 #836), 제안 5 승인 요청 알림의 Hermes 창 미러링 — `SendOnlyChannel`·`select_mirror_channel`·`announce_pending_approvals(mirror=)`, operator에 `HERMES_BOT_TOKEN` 발신 전용(PR11). Thomas의 D-1~D-6 답은 전부 권고안(a). 이 기록 밖에 남은 것: 호스트의 shim v2, 정책 1.5.0(Thomas 직접 적용), SOUL/cron 조정. 아래 본문은 제안 당시의 `path:line`을 그대로 둔다.
**Owner:** Thomas.
