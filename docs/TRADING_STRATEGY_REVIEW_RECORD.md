...
>
> **갱신 (2026-08-06): A1이 닫히는 대신 지불 불가로 판명됐고, 그래서 코드가 아니라 mint 물량이
> 바뀌었다.** A3(#438)이 홀드아웃 문을 조이고 A1(#440)이 시도 수를 랭킹 tier로 올린 뒤, 그
> 두 문을 실제 저장소에 통과시켜 봤다. 승격 문 산출은 **0**이고, 그 0은 게이트 결함이 아니라
> 측정이다. 저장소 1,681건 중 홀드아웃 블록 1,508건의 사인:
>
> | 탈락 지점 | 건수 |
> |---|---|
> | 홀드아웃 25거래 미만 | 512 (34%) |
> | `stdev_r` 없음 — 스키마 빈티지 | 533 (35%) |
> | 거래 단위 구간이 0을 못 넘음 → CONTRADICTED | 462 (31%) |
> | period 게이트 도달 | **1** |
> | CONFIRMED | **0** |
>
> **판정 가능한 463건 중 462건이 CONTRADICTED다.** #555가 넣은 period 게이트(10슬라이스 중 8)는
> 도달 자체가 1건뿐이라 이 결과에 관여하지 않는다 — 이 문서가 A3에서 걱정한 "게이트가 너무
> 약하다"는 닫혔고, 지금 남은 것은 모집단 문제다.
>
> **비용을 0으로 놓아도 열리지 않는다.** 각 후보의 홀드아웃 기대값에 그 후보 자신의 거래당
> 비용을 되돌려 gross로 바꾼 뒤 같은 게이트를 다시 돌린 결과(판정가능 463건 고정):
>
> | 비용 수준 | t≥1.96 통과 | Bonferroni 통과 |
> |---|---|---|
> | 실제 (0.2247R) | 1 | 0 |
> | 1d 수준 (0.0321R) | 5 | 0 |
> | **0 (gross)** | **6** | **0** |
>
> 순수 노이즈 모집단이 예측하는 우연 통과는 463 × 2.5% = **11.6건**이다. 관측 6건은 그보다
> **적다.** B2 갱신이 관측한 "순 기대값이 사실상 비용의 함수"는 여전히 맞지만, 비용을 없애도
> 남는 것이 없다는 뜻이기도 하다. 거래당 비용 0.2247R의 내역은 taker 0.1383 + slippage 0.0790
> + maker 0.0067 + funding 0.0006 — B1이 넣은 펀딩은 무시 가능하고 B3(슬리피지 미측정)가
> 두 번째 항목이다.
>
> **A1의 문턱이 실제로 얼마인지.** 홀드아웃 `stdev_r` 중앙값 1.363R, n=100 기준 무보정
> 문턱은 +0.267R, context당 시도 ~120건의 Bonferroni 보정 후는 **+0.481R**이다. 실측 gross
> 분포는 p50 −0.002R / **p90 +0.209R** / max +0.753R — **p90 후보조차 무보정 문턱에 미달한다.**
> 진짜 엣지 +0.05R을 보정 후 확인하려면 홀드아웃 9,208거래가 필요한데 실제 중앙값은 34거래다.
> 즉 이 게이트는 **+0.48R 이상만 확인 가능한 도구**이고, 찾는 효과보다 10배 크다.
>
> **그래서 취한 조치는 코드가 아니라 스케줄이다.** `attempts_by_context`는 읽는 시점에 저장소
> 전체를 세고 감소하지 않으므로, mint 한 건마다 같은 context의 모든 형제의 문턱이 오른다
> (미조치 시 90일 후 z 3.53 → 3.88). 2026-08-06:
>
> - **1h 티어 mint 동결** — `crypto_factory` 스케줄 5개 disable(enabled 15 → 10, 4h·1d만).
>   *(물량이 실제로 얼마나 줄었는지는 아래 정정을 볼 것 — 줄지 않았다.)*
>   근거는 진입 신호 기여도가 0이라는 것: null control 135 컨텍스트 / 15,762거래에서 실제
>   −0.1059R 대 랜덤 −0.1093R, beats-fraction 0.520. 1h는 비용 0.1515R을 그 위에 얹는다.
> - **`crypto_null_control` 스케줄 15개 신규 등록** — 1h/4h/1d × 5심볼, 일 1회, 대상 664 spec.
>   A2("홀드아웃이 후보 전체에 공유·재사용된다")에 대한 답이다: mint **이후** bar만 보므로
>   선택 편향이 구조적으로 없다. 게이트가 `MIN_POST_MINT_DAYS=30` / `MIN_POST_MINT_TRADES=10`
>   이라 첫 실측은 2026-08-28경(1h 5셀 + 4h 5셀 개방)이고, 그 전까지 `measured=0`은 누적
>   부족이지 결함이 아니다.
>
> **하지 않은 것과 그 이유.** `DEFAULT_BATCH_SIZE`(4)와 `FACTORY_FUSION_PAIRS`(4) 인하는
> 측정 후 보류했다. `selection_adjusted_z`가 `sqrt(2 ln N)`이라 mint 속도에 거의 둔감하다 —
> batch 4→2는 90일 문턱을 +0.529R에서 +0.513R로 **3.0%** 낮추는 대가로 템플릿 로테이션을
> 10일 → 19일로 두 배 늦춘다. E3(템플릿 편중)와 `context_rotation_phase`가 이미 값을 매긴
> 비용이고, 이득이 그보다 작다. **깊이를 줄이는 것은 context를 줄이는 것의 대체재가 아니다.**
> `FACTORY_FUSION_PAIRS`는 별개로 오늘 기준 무효과다: crossover 산출이 60/일(08-04까지) → 6
> → 0으로 떨어졌고, 바인딩은 pairs 상한이 아니라 부모 풀
> (`holdout_permits_parenting` 통과 218/1,681)이다. **[틀렸다 — 바로 아래 정정을 볼 것]**
>
> 스케줄은 `.runtime_governance_state/schedules.jsonl`의 per-machine 상태라 커밋되지 않는다.
> 다른 호스트에서 로테이션이 달라 보이면 그 이유이고, 되돌리기는
> `scheduler_cli enable <schedule_id>`로 대칭이다.
>
> ---
>
> **정정 (2026-08-06, 같은 날 저녁): 위 문단의 `FACTORY_FUSION_PAIRS` 서술이 틀렸다. 그리고
> 동결의 물량 효과도 틀렸다.** 둘 다 원인이 하나다 — #561(`the budget fusion declined was
> handed back to nobody`, 같은 날 04:23Z 머지)이 그 사이에 배포됐고, 위 측정은 그 이전 이미지가
> 만든 저장소를 읽은 것이다.
>
> #561 이후 `fusion_pairs`는 fusion **상한**이 아니라 발사당 mint **하한**이다. 불변식은
> `factory.py`가 직접 쓴다: `fused_count + seeded_topup_count == fusion_pairs`. fusion이 못 쓴
> 예산은 폐기되지 않고 seeded 로테이션으로 넘어간다. 부모 풀이 마를수록 4개 슬롯이 **전부**
> seeded로 전환되므로, 이 상수는 "무효과"가 아니라 지금 발사당 mint의 절반을 정한다.
>
> 08-06 08:09Z 발사에서 관측된 그대로다 — 4h 5개 컨텍스트가 `generated=8/8 fused=0 topup=4`
> (한 컨텍스트만 `7/7 fused=1 topup=3`). 발사당 4건이 아니라 8건이다.
>
> 그래서 위에 적은 물량 감소는 일어나지 않았다:
>
> | | 발사당 | 활성 스케줄 | 일일 mint |
> |---|---|---|---|
> | 동결 전 (#561 이전) | 4 | 15 | ~66 |
> | 동결 후 (#561 이후) | **8** | 10 | **~80** |
>
> 1h 동결이 뺀 ~20건보다 topup이 남은 10개 컨텍스트에 더한 ~40건이 크다. 08-06 당일 60건인
> 것은 1d가 03:35Z에 구 이미지로 돌았기 때문이고, 다음 날부터 1d도 8건씩 낸다.
>
> **그러나 결론은 바뀌지 않는다 — 오히려 강화된다.** 물량을 줄이는 것이 문턱을 못 움직인다는
> 위 논거는 topup에도 그대로 적용된다. 활성 컨텍스트(4h·1d 10개) 평균 attempts 77 기준, 90일 후:
>
> | 시나리오 | context당/일 | 90일 필요 edge | 유지 대비 |
> |---|---|---|---|
> | 유지 (fusion_pairs=4) | 8 | +0.545R | — |
> | 4 → 2 | 6 | +0.537R | 1.5% |
> | 4 → 0 | 4 | +0.526R | 3.6% |
> | mint 완전 정지 | 0 | +0.465R | 14.8% |
>
> **mint를 완전히 멈춰도 14.8%다.** 그리고 그 +0.465R조차 실측 gross p90(+0.209R)의 두 배가
> 넘는다. 시도를 1회로 줄여 보정을 완전히 없애도 무보정 문턱은 +0.267R로, 여전히 p90 위에 있다.
> 즉 **물량은 통계적 레버가 아니다.** `FACTORY_FUSION_PAIRS`를 줄일 이유가 남아 있다면 그것은
> Bonferroni가 아니라 발사당 replay 비용, 스토어 성장(H2), 스케줄러 pass 점유이며, 그 근거로
> 따로 판단할 일이다. Thomas는 2026-08-06에 유지를 택했다.
>
> 1h 동결 자체는 유지한다 — 다만 근거는 문턱이 아니라 위에 적은 그대로다: 1h는 비용 0.1515R을
