...
>
> **Operator state is not the whole of it.** The evidence those doors would be opened on is the
> other half, and it moved the wrong way this week: this runtime's own paper record is 60 closed
> trades at +0.08R with a 95% interval of `[-0.32, +0.48]` — the sign is unknown, not small
> (#292/#304) — and re-deriving the candidate store at the taker rate the venue actually charges
> flips thirteen lineages from positive expectancy to negative (#257/#260). Wired is not
> permitted; permitted would still not be advisable today.
...
- [ ] **The strategy evidence cannot state the sign of its own edge.** Routing being built does
      not make this less true — it makes it the binding item, because it is now the only one left
      that no operator action can clear. 60 closed trades at **+0.08R**, 95% interval
      `[-0.32, +0.48]`, ~0.4 standard errors from zero, and roughly **3,100 trades** to separate
      the effect as observed (#292). At ~18 closed trades a day that is months for the observed
      effect, or about a month if the true edge is the ~+0.2R the backtests claim; the distance
      between those two figures is the reason to keep collecting rather than to permit. The board
      states it in its own `판단` line rather than leaving it to be recomputed here.
