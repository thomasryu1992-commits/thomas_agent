...
**Status (2026-09-15):** Thomas decided to deploy. §3–§4 executed: `candidate-865` promoted and `up -d` at 02:31 UTC (rollback `rollback-pre-865`), the risk lane firing after it; §5 merged as #866 and applied with `up -d --no-deps dispatch-bridge` at 02:57 UTC (the door logs `workflow-manager=on`; the store was created as uid 10001; the risk lane fired at 02:59 with the same 13 contexts). §6 executed by Thomas at 03:47 UTC after the installer fix (#868 — the host's uutils `install` refused the numeric owner and the first attempt stopped before any file changed): `install_hermes_shims.sh --check` clean, `SOUL.md` and `SKILL.md` identical to the repository (one `thomas-ops` skill, 1.5.5), four cron jobs with 워크플로 서술 next at 04:00, both backup scripts identical, and the gateway registered 53 MCP tools from 4 servers — 19 on `thomas-dispatch`, the v3 workflow tools among them — with telegram connected and no error logged. The §7 repository revalidation passes; the checks that need `docker exec` into a container were not run from the session. Open: the next core backup's `workflow-snapshot=` marker, and the pilot. Policy 1.6.0 and `--v2-intake closed` remain out, as §1 says.
